  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <ScrollArea className="flex-1 overflow-auto max-h-[calc(90vh-12rem)] px-4">
          <div className="space-y-6 pb-4">
              ...
              <CardContent className="pt-6">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-medium text-lg">KES {tenant.deposit.toFixed(2)}</p>
            ...
            {tenant.balance > 0 && (
              <Alert className="text-red-500">
                ...
                <AlertTitle className="font-md text">Outstanding Balance</AlertTitle>
                <AlertDescription className="font-md">
                  Tenant has an outstanding balance of KES {tenant.balance}. This amount should be Deducted from the Deposit.
                </AlertDescription>
              </Alert>
            )}
              ...
              <CardContent className="pt-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-[1fr,2fr,1fr] gap-2">
                    ...
                    <Input placeholder="Description" value={deduction.description} onChange={(e) => setDeduction({ ...deduction, description: e.target.value })} />
                    <div className="flex gap-2">
                      <Input type="number" placeholder="Amount" value={deduction.amount || ''} onChange={(e) => setDeduction({ ...deduction, amount: parseFloat(e.target.value) || 0 })} />
                      <Button size="icon" onClick={addDeduction}><Plus className="h-4 w-4" /></Button>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input type="date" value={formData.processingDate} onChange={(e) => setFormData({ ...formData, processingDate: e.target.value })} />
                    </div>
                    <div className="space-y-2">
                  ...
                  {REFUND_METHODS.find(m => m.id === formData.refundMethod)?.needsDetails && (
                    <div className="space-y-4 pt-2">
                      <Separator />
                      <div className="grid grid-cols-2 gap-4">
                        <Input placeholder="Phone Number" value={formData.recipientDetails?.phone} onChange={(e) => setFormData({ ...formData, recipientDetails: { ...formData.recipientDetails, phone: e.target.value } })} />
                        {formData.refundMethod === 'bank_transfer' && (
                          <>
                            <Input placeholder="Bank Name" value={formData.recipientDetails?.bankName} onChange={(e) => setFormData({ ...formData, recipientDetails: { ...formData.recipientDetails, bankName: e.target.value } })} />
                            <Input placeholder="Account Number" value={formData.recipientDetails?.accountNumber} onChange={(e) => setFormData({ ...formData, recipientDetails: { ...formData.recipientDetails, accountNumber: e.target.value } })} />
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="pt-4">
                    ...
                    <Textarea value={formData.reason} onChange={(e) => setFormData({ ...formData, reason: e.target.value })} placeholder="Add any additional notes about the refund" />
                  </div>
                </div>
              </CardContent>
            </Card>
